...
class RPN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        box_cls_loss_type: str = "WCE",
        box_cls_loss_weight: float,
        ...
        Args:
            ...
            box_cls_loss_weight (float): object weight parameter for WCE and WFOCAL loss.
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret = {
            ...
            "box_cls_loss_type": cfg.MODEL.RPN.BBOX_CLS_LOSS_TYPE,
            "box_cls_loss_weight": cfg.MODEL.RPN.BBOX_CLS_LOSS_WEIGHT,
    ...
    def losses(
        ...
        if self.box_cls_loss_type == "CE":
            ...
            weight = gt_labels[valid_mask].to(torch.float32) * self.box_cls_loss_weight
            objectness_loss = F.binary_cross_entropy_with_logits(
                cat(pred_objectness_logits, dim=1)[valid_mask],
                gt_labels[valid_mask].to(torch.float32), weight=weight,
                reduction="sum",
            )
        elif self.box_cls_loss_type == "WFOCAL":
            objectness_loss = sigmoid_focal_loss_jit(
                cat(pred_objectness_logits, dim=1)[valid_mask],
                gt_labels[valid_mask].to(torch.float32),
                alpha=1 - self.box_cls_loss_weight/(self.box_cls_loss_weight + 1),
            ...
            losses = {
                "loss_rpn_cls": objectness_loss / self.loss_normalizer,
